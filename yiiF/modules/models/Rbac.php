...
class Rbac extends ActiveRecord 
{
        ...
        try {
            ...
            foreach ($children as $item) {
                ...
                if(!$auth->getRole($item)){
                    $obj = $auth->getPermission($item);
                }else{
                    $obj = $auth->getRole($item);
                };
    ...
    private static function getItemByUser($adminid, $type)
    {
    ...
    {
        $return = [];
        $return['roles'] = self::getItemByUser($adminid, 1);
        $return['permissions'] = self::getItemByUser($adminid, 2);
        return $return;
    }
