...
{
    protected $mustlogin = [
        'index','check','add','confirm','pay','getexpress','received'
    ];
    protected $verbs=[
        'confirm'=>['post']
    ];
        ...
        try {
            if (Yii::$app->request->isPost) {
                ...
                foreach ($post['OrderDetail'] as $product) {
                    $shopproduct = Product::find()->where('productid = :proid', [':proid' => $product['productid']])->one();
                    if ($product['productid'] > $shopproduct->num) {
                        Yii::$app->session->setFlash( 'info',"<script>alert(\"{$shopproduct->title}库存不足，请重新输入！\")</script>");
