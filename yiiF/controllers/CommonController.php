...
class CommonController extends Controller
{
	...
	if(!$data=$cache->get($key)){
		...
		$dep2=new \yii\caching\DbDependency([	
		
		'sql'=>'select max(updatetime) from {{%product}} where ison = "1"',
		
		]);
	$tui=Product::getDb()->cache(function(){
	return  $tui = Product::find()->where('istui = "1" and ison = "1"')->orderby('createtime desc')->limit(3)->all();
	},60,$dep2);
$new=Product::getDb()->cache(function(){
	return  $new = Product::find()->where('ison = "1"')->orderby('createtime desc')->limit(3)->all();
        	},60,$dep2);
$hot=Product::getDb()->cache(function(){
	return  $hot = Product::find()->where('ison = "1" and ishot = "1"')->orderby('createtime desc')->limit(3)->all();
        	},60,$dep2);
$sale=Product::getDb()->cache(function(){
	return   $sale = Product::find()->where('ison = "1" and issale = "1"')->orderby('createtime desc')->limit(3)->all();
      	},60,$dep2);
